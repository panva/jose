...
const expectFailure = [
  ['EdDSA', '{"kty":"OKP","crv":"Ed25519","x":"UX-2D7zxYLGioHoM-SUVpeFFNo9P7YikFGr2dh0S0R8"}'],
  [
    'EdDSA',
    '{"kty":"OKP","crv":"Ed25519","x":"UX-2D7zxYLGioHoM-SUVpeFFNo9P7YikFGr2dh0S0R8","d":"pjhYsOQ8x2N6d3rekqpTeD0yeXIZIkiIPX7Q3QjXjko"}',
  ],
  ['ECDH-ES', '{"kty":"OKP","crv":"X25519","x":"IgTW3WUetorvpyWIMX0MK7zVkFebI6nOFQ5315TLKWs"}'],
  [
    'ECDH-ES',
    '{"kty":"OKP","crv":"X25519","x":"IgTW3WUetorvpyWIMX0MK7zVkFebI6nOFQ5315TLKWs","d":"8GzXT_MJgUXdXnpM8V9Oh9szRPuZ1ghjZ9Qu1ds4N3g"}',
  ],
  [
    'EdDSA',
    '{"kty":"OKP","crv":"Ed448","x":"P4VT2Jk8ft0cgpC43mQX5OcaXE6VKXfkKMGA0qkeGhmPR2yr9Z7UYSzs5QiFREeFFzkbhyOgHDEA"}',
  ],
  [
    'EdDSA',
    '{"kty":"OKP","crv":"Ed448","x":"P4VT2Jk8ft0cgpC43mQX5OcaXE6VKXfkKMGA0qkeGhmPR2yr9Z7UYSzs5QiFREeFFzkbhyOgHDEA","d":"k28CQuDvByCLBQ7aZvlzApTod9skzMtVVN39aHLYjCBJVK27MfAmVaJ4qxqN-AU3LKkT60jEGI7a"}',
  ],
  [
    'ECDH-ES',
    '{"kty":"OKP","crv":"X448","x":"KErL1kZ1r8Onb6DbrfQSOP34o1h7AWYRNNGTnXmf_z4AMOQqwbIKFQztADNEcGLI27nz_r9618E"}',
  ],
  [
    'ECDH-ES',
    '{"kty":"OKP","crv":"X448","x":"KErL1kZ1r8Onb6DbrfQSOP34o1h7AWYRNNGTnXmf_z4AMOQqwbIKFQztADNEcGLI27nz_r9618E","d":"2Kz7WV4t8Ffkup_f26i70BsMFZClN47GaXuueM-xkNraXtcRyqVrp0i1Uh5axz_KdbzLMZHgjew"}',
  ],
  [
    'ES256K',
    '{"kty":"EC","crv":"secp256k1","x":"Z2jPhON0yh9F5N4DoUzvVX6tfNtDjmwdqpodR9pX-NI","y":"grTNHdY88Mo6UbKA7-z4n6BgA9v3dZ-yewwbLEsXTYQ"}',
  ],
  [
    'ES256K',
    '{"kty":"EC","crv":"secp256k1","x":"Z2jPhON0yh9F5N4DoUzvVX6tfNtDjmwdqpodR9pX-NI","y":"grTNHdY88Mo6UbKA7-z4n6BgA9v3dZ-yewwbLEsXTYQ","d":"B0ed4c2LWiG67o6MdYgYWeo_8gHYyRCQ3xWR0hYM41c"}',
  ],
]

for (const [alg, jwk] of expectSuccess) {
  const { crv, d, k } = JSON.parse(jwk)
  QUnit.test(
    `${d ? 'Private' : k ? 'Secret' : 'Public'} JWK Import/Export ${crv || alg}`,
    test.bind(undefined, JSON.parse(jwk), alg),
  )
}
...
for (const [alg, jwk] of expectFailure) {
  const { crv, d, k } = JSON.parse(jwk)
  QUnit.test(
    `(expecting failure) ${d ? 'Private' : k ? 'Secret' : 'Public'} JWK Import/Export ${
      crv || alg
    }`,
