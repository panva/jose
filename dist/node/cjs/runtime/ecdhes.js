...
const deriveKey = async (publicKee, privateKee, algorithm, keyLength, apu = new Uint8Array(0), apv = new Uint8Array(0)) => {
    let publicKey;
    if (webcrypto_js_1.isCryptoKey(publicKee)) {
        publicKey = webcrypto_js_1.getKeyObject(publicKee, 'ECDH-ES');
    }
    else if (is_key_object_js_1.default(publicKee)) {
        publicKey = publicKee;
    }
    else {
        throw new TypeError(invalid_key_input_js_1.default(publicKee, 'KeyObject', 'CryptoKey'));
    }
    let privateKey;
    if (webcrypto_js_1.isCryptoKey(privateKee)) {
        privateKey = webcrypto_js_1.getKeyObject(privateKee, 'ECDH-ES', new Set(['deriveBits', 'deriveKey']));
    }
    else if (is_key_object_js_1.default(privateKee)) {
        privateKey = privateKee;
    }
    else {
        throw new TypeError(invalid_key_input_js_1.default(privateKee, 'KeyObject', 'CryptoKey'));
    }
