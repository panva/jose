...
test(`${recipe.title} - compact verify`, t => {
  t.deepEqual(JWS.verify(recipe.output.compact, key, { crit: ['b64'] }), Buffer.from(payload))
})
...
;[keystoreMatchOne, keystoreMatchMore].forEach((keystore, i) => {
  test(`${recipe.title} - compact verify (using keystore ${i + 1}/2)`, t => {
    t.deepEqual(JWS.verify(recipe.output.compact, keystore, { crit: ['b64'] }), Buffer.from(payload))
  })
