...
const { JWK, JWS, errors: { JWSInvalidHeader } } = require('../..')
...
test('b64 must be integrity protected', t => {
  ...
  }, { instanceOf: JWSInvalidHeader, code: 'ERR_JWS_INVALID_HEADER', message: '"b64" critical parameter must be integrity protected' })
})

test('b64 must be a boolean', t => {
  t.throws(() => {
    JWS.sign.flattened('foo', k, { alg: 'HS256', crit: ['b64'], b64: 'true' })
  }, { instanceOf: JWSInvalidHeader, code: 'ERR_JWS_INVALID_HEADER', message: '"b64" critical parameter must be a boolean' })
})
...
test('b64 must be the same for all recipients', t => {
  ...
  t.throws(() => {
    sign.sign('general')
  }, { instanceOf: JWSInvalidHeader, code: 'ERR_JWS_INVALID_HEADER', message: 'the "b64" Header Parameter value MUST be the same for all recipients' })
})
