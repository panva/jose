...
This section documents the threat model for `jose`, a JavaScript implementation of JSON Object Signing and Encryption standards including [JSON Web Token (JWT) - RFC 7519](https://www.rfc-editor.org/info/rfc7519/), [JSON Web Signature (JWS) - RFC 7515](https://www.rfc-editor.org/info/rfc7515/), [JSON Web Encryption (JWE) - RFC 7516](https://www.rfc-editor.org/info/rfc7516/), [JSON Web Key (JWK) - RFC 7517](https://www.rfc-editor.org/info/rfc7517/), and [JSON Web Algorithms (JWA) - RFC 7518](https://www.rfc-editor.org/info/rfc7518/). It also provides the cryptographic primitives of [HTTP Message Signatures - RFC 9421](https://www.rfc-editor.org/info/rfc9421/).
...
- **HTTP Message Signature primitives**: The `jose/httpsig` API implements only the `HTTP_SIGN` and `HTTP_VERIFY` cryptographic primitives of RFC 9421 over caller-supplied bytes. `sign` is an unrestricted signing oracle; signing attacker-influenced data is application misuse. Constructing the signature base and enforcing its ASCII-only requirement, selecting and validating covered components, `created`, `expires`, `nonce`, and `tag` policy, replay prevention, resolving `keyid` to a trusted key, and determining the algorithm from the application's own key registration rather than from the message are application responsibilities. Treating a `true` result from `verify` as authentication of an HTTP message is application misuse.
