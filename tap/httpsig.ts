...
export default (
  ...
  async function keysFor(
    joseAlg: string,
  ): Promise<{ signingKey: jose.CryptoKey; verificationKey: jose.CryptoKey }> {
    if (joseAlg.startsWith('HS')) {
      // Only the AES_CBC_HMAC_SHA2 identifiers yield a Uint8Array, HS* always yields a CryptoKey
      const secret = (await lib.generateSecret(joseAlg)) as jose.CryptoKey
