...
const { JWEInvalidHeader, JWEInvalid, JOSENotSupported } = require('../errors')
const validateCrit = require('../help/validate_crit').bind(undefined, JWEInvalid)
...
module.exports = (prot, unprotected, recipients, checkAlgorithms, crit) => {
  ...
  if (!isDisjoint(prot, unprotected) || !recipients.every(({ header }) => {
    ...
    if ('zip' in combined) {
      throw new JWEInvalidHeader('"zip" Header Parameter MUST be integrity protected')
  ...
  if (checkAlgorithms) {
    if (alg.length !== recipients.length) {
      throw new JWEInvalidHeader('missing Key Management algorithm')
    }
    if (enc.size === 0) {
      throw new JWEInvalidHeader('missing Content Encryption algorithm')
    } else if (enc.size !== 1) {
      throw new JWEInvalidHeader('there must only be one Content Encryption algorithm')
    }
  } else {
    if (enc.size > 1) {
      throw new JWEInvalidHeader('there must only be one Content Encryption algorithm')
