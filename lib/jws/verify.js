...
const jwsVerify = (skipDisjointCheck, serialization, jws, key, { crit = [], complete = false, algorithms } = {}) => {
  ...
  if (SINGLE_RECIPIENT.has(serialization)) {
    ...
    if (!skipDisjointCheck && !isDisjoint(parsedProt, header)) {
      throw new errors.JWSInvalidHeader('JWS Protected and JWS Unprotected Header Parameter names must be disjoint')
    }
    ...
    if (!alg) {
      throw new errors.JWSInvalidHeader('missing JWS signature algorithm')
