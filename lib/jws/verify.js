...
const { JWSVerificationFailed, JWSInvalidHeader, JWSInvalid } = require('../errors')
...
const jwsVerify = (skipDisjointCheck, serialization, jws, key, { crit = [] } = {}) => {
    ...
    if (key instanceof KeyStore) {
      ...
      if (keys.length === 1) {
        ...
        for (const key of keys) {
          try {
            return jwsVerify(true, serialization, jws, key, { crit })
          } catch (err) {
  ...
  for (const recipient of signatures) {
    try {
      return jwsVerify(false, 'flattened', { ...root, ...recipient }, key, { crit })
    } catch (err) {
      continue
    }
  }
