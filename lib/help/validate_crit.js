const { JOSECritNotUnderstood, JWSInvalidHeader } = require('../errors')
    ...
    protectedHeader.crit.forEach((parameter) => {
      ...
      if (parameter === 'b64') {
        if (!('b64' in protectedHeader)) {
          throw new JWSInvalidHeader('"b64" critical parameter must be integrity protected')
        }
        if (typeof protectedHeader.b64 !== 'boolean') {
          throw new JWSInvalidHeader('"b64" critical parameter must be a boolean')
        }
